...
class Gaia extends DB{
	...
	public function __construct($database='',$DOMAIN='',$dbt='maria'){	
				$this->my=$_COOKIE;
				//$globalfile=SITE_ROOT."globals.json";
				//if(file_exists($globalfile)){
					//$this->GS = jsonget($globalfile);	
				//}elseif($_SERVER['PHP_SELF']!='/gaia/dsh/setupajax.php' && !AJAXREQUEST){					
					
					$this->GS=array();
				$this->GS['CURRENT']= getcwd();
				$this->GS['URL']= php_sapi_name() !='cli' ? SITE_URL.$_SERVER['REQUEST_URI']:'';
				$this->GS['URL_FILE']= URL_FILE;
				$this->GS['URL_PAGE']= basename(URL_FILE, ".php");
				$this->GS['SELF']= php_sapi_name() !='cli' ? (SITE_URL.$_SERVER['PHP_SELF'].($_SERVER['QUERY_STRING']!="" ? '?'.$_SERVER['QUERY_STRING']:'')):'';
				$this->GS['SELF_NONURL']= php_sapi_name() !='cli' ? $_SERVER['PHP_SELF'].($_SERVER['QUERY_STRING']!="" ? '?'.$_SERVER['QUERY_STRING']:''):'';
				$this->GS['QUERY_STRING']= php_sapi_name() !='cli' ? $_SERVER['QUERY_STRING'] :'';
				$this->GS['server']= $_SERVER;
				$this->GS['CUR_DIR']= basename(dirname($_SERVER['PHP_SELF']));
				
					$setupjson=GAIABASE.'gaia.json';
					if(file_exists($setupjson)){
//xecho($_SERVER['SERVER_NAME']);						
					$this->db=new DB("gs_".DOMAINAME.DOM_EXT,$dbt);	
					include GAIAROOT."globals.php";		
					}
					
				//echo(json_encode($this->GS,JSON_PRETTY_PRINT));
				//	file_put_contents($globalfile,json_encode($this->GS,JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), FILE_APPEND | LOCK_EX);
				//}		
				foreach($_GET as $get =>$getval){
					$this->GS[$get] =trim($getval);
				}							
								
				$this->GS['page']= isset($_GET['page']) ? trim($_GET['page']):'';
				$this->GS['mode']= isset($_GET['mode']) ? trim($_GET['mode']):'';				
				$this->GS['sub']= isset($_GET['sub']) ? trim($_GET['sub']):'';
				$this->GS['id']= isset($_GET['id']) ? trim($_GET['id']):'';				
				$this->GS['uid']= isset($_GET['uid']) ? trim($_GET['uid']):'';				
				
				$getpage= $this->GS['page']!='' ?  $this->GS['page'] : 'index';
				
				//URI RULES 
				//global pages
				if(in_array($getpage,array('login','register'))) {
					$this->GS['paget']= GAIAROOT.$getpage.".php";
					$this->GS['pagetype'] ="global";
				//template default pages
				}elseif($getpage=="index"){
					$this->GS['paget']= $this->GS['PAGESURI']."$getpage.php";
					$this->GS['pagetype'] =$getpage;
				}elseif(in_array($getpage,array("app","post","tax","tag","search","user"))){
					if($this->GS['mode']!=""){
					$this->GS['paget']= $this->GS['PAGESURI']."$getpage.php";
					}else{
					$this->GS['paget']= $this->GS['PAGESURI']."archieve.php";	
					}
					$this->GS['pagetype'] =$getpage;				
									
				}elseif(in_array($getpage,$this->GS['tax_uri'])){
					$this->GS['paget']= $this->GS['PAGESURI']."archieve.php";	
				//}elseif(in_array($getpage,$this->GS['static_pages'])){
//					$this->GS['paget']= $this->GS['PAGESURI']."$getpage.php";
					//$this->GS['pagetype'] ="static";
				}else{
					$this->GS['paget']= $this->GS['PAGESURI']."page.php";
					if($this->GS['page']!=""){$this->GS['data']= $this->db->f("SELECT * FROM post where uri=?",array($this->GS['page'])); 
					$this->is['logo']=$this->GS['data']['img'];
					}
					$this->GS['pagetype'] ="404";
				}
				//RUN every GS key as public var of Gaia
				foreach($this->GS as $gsname =>$gsval){
					$this->$gsname = $gsval;
				}	
    }
	/*
	 ...
	 * $this->GS['GET']['page']==dsh or not (public)
 ...
 * */
	...
	public function template_new($template,$onlyinsert=false){
        ...
        $sfolders=array('pages','widgets','css','js','images','preview');
        ...
        //page system
        $pagefolder= $this->TEMPLATES.$template."/pages/";
    ...
    public function template_activate($name){        
		...
		if(!file_exists($this->TEMPLATESURI . $name ."/pages")){
		mkdir($folder, 0777, true);
		//2 copy *.html to pages
		$htmls = glob($this->TEMPLATESURI . $name . '/*.html');
        if (!empty($htmls)) {
            foreach ($htmls as $html) {
                $htmlname = explode('.', basename($html))[0];
                $phpname = dirname($html) . "/pages/$htmlname.php";
                //copy php to pages
                copy($html, $phpname);chmod($phpname, 0777);
                //move *.html to preview
                rename($html, $this->TEMPLATESURI . "$name/preview/" . basename($html)); //copy to preview
            }            
        }
		}
		...
		//4 update globs		
		return $this->db->q("UPDATE globs SET en=? WHERE name='template_active'",array($name));		
    }
...
//                $col= $typ2table.' '.$typ2row;
                ...
                <link href='/gaia/lib/uploader/css/jquery.fileupload.css' rel=\"stylesheet\" type=\"text/css\" />                
				  ...
				  <button id=\"selectimage\" onclick=\"g.media.select(this)\" class=\"btn btn-info btn-sm\">Select</button>
                  </div>";
                }elseif($typ1=='select') {
